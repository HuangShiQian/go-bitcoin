...
type Transaction struct{
	...
	TimeStamp uint64   //创建交易时间
}

type TXInput struct{
	Txid []byte  //这个input所引用的output所在的交易id
	Index int64 //这个input所引用的output所在的交易的索引
	//ScriptSig string  //付款人对当前交易（是新交易TXID对应的，而不是引用的交易）的签名
	ScriptSig []byte //对当前交易的签名
	PubKey    []byte //付款人的公钥
}

type TXOutput struct{
	ScriptPubKeyHash []byte  //收款人的公钥哈希  先理解为地址
	Value float64  //转账金额
}
...
func NewCoinbaseTx(miner /*挖矿人*/ string,data string)*Transaction  {
	...
	//output:=TXOutput{ScriptPubk:miner,Value:reward}
	//创建output
