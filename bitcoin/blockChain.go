...
func CreateBlockChain(address string) error {
	...
	if utils.IsFileExist(blockchainDBFile) {
	...
	db.Update(func(tx *bolt.Tx) error {
		...
		if bucket==nil{
			...
			bucket.Put(genesisBlock.Hash,genesisBlock.Serialize())//将block进行序列化
			//更新最后区块哈希值到数据库
...
func GetBlockChainInstance() (*BlockChain,error) {
	...
	if utils.IsFileExist(blockchainDBFile) == false {
...
func (bc *BlockChain)AddBlock(txs1 []*Transaction) error {
	...
	lastBlockHash := bc.Tail //区块链中最后一个区块的哈希值
	//1. 创建区块
	...
	err := bc.Db.Update(func(tx *bolt.Tx) error {
		...
		bucket.Put([]byte(lastBlockHashKey), newBlock.Hash)//更新最后一个区块的hash
...
type Iterator struct {
	DB *bolt.DB
	CurrentHash []byte   //不断移动的哈希值  用于访问所有区块
}

//将Iterator 绑定到 BlockChain
...
// 1.返回当前所指向的区块
// 2.向左移动 （指向前一个区块）
func (it *Iterator) Next()(block *Block)  { //注意：返回值这里要是写了名字，则在后面不用:=了，返回也只要return就行了
	//读取 Bucket 当前 哈希Block
	err:=it.DB.View(func(tx *bolt.Tx) error {
		...
		blockTmpInfo/*block的字节流*/:=bucket.Get(it.CurrentHash)
